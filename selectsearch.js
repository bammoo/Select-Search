...
var searchSelectBaloon = {
  ...
  init: function (textbrut, text, e) {
    ...
    addEvent(
      searchSelectBaloon.baloon,
      "mouseover",
      searchSelectBaloon.baloonOver
    );

    //Clipboard stuff
    if (document.getElementById("selectsearchcpboard")) {
      chrome.extension.sendMessage(
        { options: "setcp", text: textbrut },
        function (response) {}
      );
      addEvent(
        document.getElementById("selectsearchcpboard"),
        "click",
        searchSelectBaloon.cpToClipboard
      );
    }
  ...
  cpToClipboard: function () {
    ...
    setTimeout(function () {
      $("#selectsearchcpboard").attr(
        "style",
        "width: 20px !important; margin:2px !important;"
      );
    }, 150);
  },
  ...
  setwidth: function () {
    ...
    document.getElementById("SearchSelectBaloon").style.width = s + 2 + "px";
  ...
  findGoodPosition: function (e) {
    ...
    if (
      searchSelectBaloon.x +
        searchSelectBaloon.size +
        Math.floor(searchSelectBaloon.size / 2) >
      window.innerWidth + window.pageXOffset
    )
      searchSelectBaloon.x =
        window.innerWidth +
        window.pageXOffset -
        (searchSelectBaloon.size + Math.floor(searchSelectBaloon.size / 2) - 3);
    var d = searchSelectBaloon.height;
    if (searchSelectBaloon.y + d > window.innerHeight + window.pageYOffset)
      searchSelectBaloon.y = window.innerHeight + window.pageYOffset - d;
  },
  ...
  baloonOut: function () {
    searchSelectBaloon.isHover = false;
    searchSelectBaloon.inactive = window.setTimeout(
      "searchSelectBaloon.fade(" + searchSelectBaloon.minTransparency + ")",
      searchSelectBaloon.fadeTime
    );
  ...
  fade: function (opac) {
    ...
    if (Math.abs(currentOpac - opac) > 10) {
      ...
      searchSelectBaloon.inactive = window.setTimeout(
        "searchSelectBaloon.fade('" + opac + "')",
        20
      );
    } else {
      searchSelectBaloon.setOpacity(opac);
      if (opac < currentOpac) {
        //if we just decided to have the baloon inactive (and fading ended)
        searchSelectBaloon.inactive = window.setTimeout(
          "searchSelectBaloon.remove()",
          searchSelectBaloon.removeTime
        ); //Remove the baloon
      }
    }
  },
  ...
  mouseDown: function (e) {
    if (searchSelectBaloon.isHover) {
      searchSelectBaloon.away = window.setTimeout(
        "searchSelectBaloon.baloon.style.visibility='hidden';",
        1000
      );
    }
  },
